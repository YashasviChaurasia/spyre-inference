#!/usr/bin/env python3
"""
Setup vLLM benchmark configs for a specific device/model combination.

Copies the relevant benchmark configs from the source directory into vLLM's
expected .buildkite/performance-benchmarks/tests/ directory structure.
"""
...
def parse_args() -> Any:
    ...
    parser.add_argument(
        ...
        action=ValidateDir,
        help="the source directory containing all vLLM benchmark configs",
        required=True,
    )
    parser.add_argument(
    ...
    for file in glob.glob(f"{from_benchmark_configs_dir}/{device}/*.json"):
        ...
        for config in configs:
            ...
            if "model" not in benchmark_config:
                warning(f"Model name not set in {benchmark_config}, skipping...")
                continue
